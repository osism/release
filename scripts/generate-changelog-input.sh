...
#
# The changelog-input files (and their batch files) are working files for
# the Claude run. Once the entry has been written to CHANGELOG.md they are
# deleted again; --keep-input keeps them for inspection. With --no-run the
# files are the actual result and are always kept.
#
...
#   -k, --keep-input   Keep the generated input/batch files after the run
...
show_help() {
    ...
    echo "  -k, --keep-input   Keep the generated input/batch files after the run"
